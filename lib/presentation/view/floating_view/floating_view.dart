import 'package:flutter/material.dart';
import 'package:sizer/sizer.dart';
import 'package:taxizer/presentation/widget/button_fc.dart';
import '../../style/style.dart';
import '../ithem_bulider_location_view/ithem_bulider_location.dart';

class FloatView extends StatelessWidget {
  final GlobalKey<FormState> formKey;
  final TextEditingController myMap;
  final TextEditingController location;
  final cubit;
  final system;
  const FloatView(
      {Key? key,
      required this.formKey,
      required this.myMap,
      required this.location,
      this.cubit,
      this.system})
      : super(key: key);

  @override
  Widget build(BuildContext context) {
    var directions;
    return Wrap(
      children: [
        Form(
          key: formKey,
          child: Column(
            mainAxisAlignment: MainAxisAlignment.center,
            mainAxisSize: MainAxisSize.min,
            children: [
              Container(
                margin: EdgeInsets.only(top: 2.h),
                width: 100.w,
                height: 25.h,
                color: backgroundcolor,
                child: Column(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                    Container(
                      width: 90.w,
                      height: 6.h,
                      decoration: BoxDecoration(
                          boxShadow: const [BoxShadow(color: wcolor)],
                          borderRadius: BorderRadius.circular(10.sp)),
                      child: TextFormField(
                        controller: myMap,
                        keyboardType: TextInputType.text,
                        textDirection: TextDirection.rtl,
                        textAlign: TextAlign.start,
                        decoration: InputDecoration(
                          contentPadding: EdgeInsets.all(6.sp),
                          border: InputBorder.none,
                          hintTextDirection: TextDirection.rtl,
                          hintText: 'موقعي الحالي',
                          suffixIcon: IconButton(
                            onPressed: () async {
                              await cubit
                                  .requestLocationPermission(context)
                                  .then((value) => cubit.getCurrentLocation())
                                  .then((value) {
                                myMap.text = cubit.address;
                                return cubit.getLocation();
                              });
                               directions = await system.fetchDirection(
                                  origin: myMap.text,
                                  destination: location.text);
                              cubit.gotoLocationUserOne(
                                  place: directions['start_location']);
                              cubit.gotoLocationUserTwo(
                                  place: directions['end_location']);
                              cubit.setPolyline(directions['polyline_decode']);
                            },
                            icon: const Icon(
                              Icons.my_location,
                              color: ycolor,
                            ),
                          ),
                        ),
                      ),
                    ),
                    Container(
                      margin: EdgeInsets.all(5.sp),
                      width: 90.w,
                      height: 6.h,
                      decoration: BoxDecoration(
                          boxShadow: const [BoxShadow(color: wcolor)],
                          borderRadius: BorderRadius.circular(10.sp)),
                      child: TextFormField(
                          controller: location,
                          keyboardType: TextInputType.text,
                          textDirection: TextDirection.rtl,
                          textAlign: TextAlign.start,
                          decoration: InputDecoration(
                            contentPadding: EdgeInsets.all(6.sp),
                            border: InputBorder.none,
                            hintTextDirection: TextDirection.rtl,
                            hintText: 'الي اين نحن ذاهبون؟',
                            suffixIcon: const Icon(
                              Icons.location_on_outlined,
                              color: ycolor,
                            ),
                          )),
                    ),
                    Row(
                      mainAxisAlignment: MainAxisAlignment.center,
                      children: [
                          Text("", maxLines: 1, style: TextStyle(fontSize: 10.sp, color: Colors.black, fontWeight: FontWeight.w600)),
                        Padding(
                          padding: EdgeInsets.all(2.sp),
                          child: Text(" :وقت الوصول",maxLines: 1,style: TextStyle(fontSize: 10.sp,color: Colors.black,fontWeight: FontWeight.w600),),
                        ),
                          Text("", maxLines: 1, style: TextStyle(fontSize: 10.sp, color: Colors.black, fontWeight: FontWeight.w600)),
                        Text(" :المسافة",maxLines: 1,style: TextStyle(fontSize: 10.sp,color: Colors.black,fontWeight: FontWeight.w600),)
                      ],
                    ),
                    SizedBox(
                      width: 100.w,
                      height: 5.h,
                      child: Row(
                        mainAxisAlignment: MainAxisAlignment.center,
                        children: [
                          ButtonFc(
                            onpres: () async {
                               directions = await system.fetchDirection(
                                  origin: myMap.text,
                                  destination: location.text);
                              cubit.gotoLocationUserOne(
                                  place: directions['start_location']);
                              cubit.gotoLocationUserTwo(
                                  place: directions['end_location']);
                              cubit.setPolyline(directions['polyline_decode']);
                            },
                            Boxcolor: ycolor,
                            width: 15.w,
                            height: 4.h,
                            child: const Text(
                              "البحث",
                              style: TextStyle(color: Colors.white),
                            ),
                          ),
                        ],
                      ),
                    ),
                    SizedBox(
                      width: 90.w,
                      child: Divider(
                        color: Colors.black45.withOpacity(0.5),
                        thickness: 1.sp,
                      ),
                    ),
                  ],
                ),
              ),
              SizedBox(
                width: 100.w,
                height: 35.h,
                child: const ItemBuilderLocation(),
              )
            ],
          ),
        ),
      ],
    );
  }
  
}
